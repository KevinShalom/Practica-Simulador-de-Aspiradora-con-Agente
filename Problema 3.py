...
class SimuladorAspiradoraConPenalizacion:
    ...
    def mover(self, direccion):
        ...
        self.movimientos += 1
        self.puntuacion -= 1  # Penalizar con -1 por cada movimiento
...
class AgenteReactivoConPenalizacion:
...
def simular_agente_con_penalizacion_en_varias_configuraciones(num_simulaciones, filas, columnas):
    ...
    for i in range(num_simulaciones):
        print(f"\n--- Simulación {i + 1} ---")
        simulador = SimuladorAspiradoraConPenalizacion(filas, columnas)
        agente = AgenteReactivoConPenalizacion(simulador)
        agente.ejecutar()
